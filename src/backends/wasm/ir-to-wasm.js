...
class IRToWasmCompiler {
  ...
  compileFunction(funcDecl) {
    ...
    func.locals = Array.from(this.localMap.entries())
      .filter(([_name, idx]) => idx >= funcDecl.parameters.length)
      .map(([, idx]) => ({
        index: idx,
  ...
  compileBinaryOp(binOp) {
    ...
    // Generate operation with reduced cyclomatic complexity
    const opMap = {
      "+": WasmOp.I32_ADD,
      "-": WasmOp.I32_SUB,
      "*": WasmOp.I32_MUL,
    ...
    const wasmOp = opMap[binOp.operator];
    if (wasmOp) {
      code.push(wasmOp);
    } else {
