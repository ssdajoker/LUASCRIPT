...
function debug_tools.debugger:inspect_variable(name)
    local level = 2
    while true do
...
function debug_tools.debugger:get_call_stack()
    local stack = {}
    local level = 2
...
function debug_tools.profiler:print_report(report)
    print("\n=== PROFILING REPORT ===")
...
function debug_tools.memory_analyzer:print_comparison(comparison)
    print("\n=== MEMORY COMPARISON ===")
...
function debug_tools.coverage:stop()
    ...
    for file, lines in pairs(self.line_hits) do
...
function debug_tools.coverage:print_coverage_report(report)
    ...
    for file, lines in pairs(report.file_coverage) do
        ...
        print("  " .. file .. ": " .. string.format("%.1f", file_percentage) .. "% (" .. file_covered .. "/" .. file_total .. ")")
    end
end
