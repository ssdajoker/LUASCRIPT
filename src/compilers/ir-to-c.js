...
class IRToCGenerator {
  ...
  visitFunctionDecl(node) {
    ...
    const parameters = node.parameters.map(param => {
      ...
      return this.formatParameter(param.name, type);
    }).join(", ");
    this.indentLevel++;
    const body = this.emitBlockStatements(node.body.statements || []);
    this.indentLevel--;
    this.popScope();
    return `${returnType} ${node.name || "__anonymous"}(${parameters || "void"})\n{\n${body}\n}`;
  }
  ...
  visitVarDecl(node) {
    if (node.init && node.init.kind === NodeCategory.ARRAY_LITERAL) {
      const arrayType = this.arrayElementType(node);
      this.setVarType(node.name, `${arrayType}[]`);
      this.setArrayLength(node.name, node.init.elements.length);
      return `${this.indent()}${arrayType} ${node.name}[] = { ${node.init.elements.map(element => this.visit(element)).join(", ")} };`;
    }
  ...
  visitExpressionStmt(node) {
    if (this.isConsoleLog(node.expression)) {
      return this.emitPrintStatement(node.expression.args);
    }
  ...
  visitCall(node) {
    ...
    if (this.isCLikeBuiltin(node, "len")) {
      return this.emitLengthExpression(node.args[0]);
    }
    if (callee === "math.abs") return `fabs(${args})`;
    if (callee === "math.ceil") return `ceil(${args})`;
    if (callee === "math.floor") return `floor(${args})`;
    if (callee === "math.max") return `fmax(${args})`;
    if (callee === "math.min") return `fmin(${args})`;
    if (callee === "math.pow") return `pow(${args})`;
    if (callee === "math.sqrt") return `sqrt(${args})`;
    return `${callee}(${args})`;
  }
  ...
  visitArrayLiteral(node) {
    return `{ ${node.elements.map(element => element ? this.visit(element) : "0").join(", ")} }`;
  }
  ...
  visitObjectLiteral(node) {
    ...
    const fields = record.fields.map(field => {
      const value = valuesByName.has(field.name) ? valuesByName.get(field.name) : this.defaultValueForType(field.type);
  ...
  emitPrintStatement(args) {
    ...
    for (const arg of args || []) {
      ...
      values.push(this.visit(arg));
    }
    const format = `${formats.join(" ")}\n`;
    return `${this.indent()}printf(${JSON.stringify(format)}${values.length ? `, ${values.join(", ")}` : ""});`;
  }
  ...
  declarationType(node) {
    ...
    if (metadataType && metadataType.endsWith("[]")) return "double";
    if (metadataType) return metadataType;
    return this.expressionType(node.init) === "string" ? "const char *" : "double";
  }
  ...
  expressionType(node) {
    ...
    if (node.kind === NodeCategory.IDENTIFIER) {
      const type = this.lookupVarType(node.name);
      if (type && type.includes("char")) return "string";
  ...
  typeFromMetadata(metadata = {}) {
    const sourceType = metadata.cType || metadata.cReturnType || metadata.cppType ||
      metadata.cppReturnType || metadata.csharpType || metadata.csharpReturnType;
  ...
  arrayElementType(node) {
    const metadataType = this.typeFromMetadata(node.metadata);
    if (metadataType && metadataType.endsWith("[]")) return metadataType.slice(0, -2);
    return "double";
  }

  formatParameter(name, type) {
    if (type.endsWith("[]")) return `${type.slice(0, -2)} ${name}[]`;
    return `${type} ${name}`;
  }

  emitLengthExpression(node) {
    if (node && node.kind === NodeCategory.IDENTIFIER) {
      const name = node.name;
      const type = this.lookupVarType(name);
      if (type && type.endsWith("[]")) {
        const length = this.lookupArrayLength(name);
        if (typeof length === "number") return String(length);
        throw new Error(`Unsupported C output feature: array length for parameter ${name} requires an explicit count`);
      }
      if (type === "const char *") return `strlen(${name})`;
    }
    throw new Error("Unsupported C output feature: dynamic length outside known arrays and strings");
  }

  normalizeScalarType(typeName) {
    if (this.recordTypes.has(typeName)) return typeName;
    if (["char*", "const char*", "std::string", "string"].includes(typeName)) return "const char *";
  ...
  defaultInitializer(type) {
    if (type === "const char *") return " = \"\"";
    if (type === "bool") return " = false";
    if (this.recordTypes.has(type)) return ` = (${type}){ 0 }`;
    return " = 0";
  }

  defaultValueForType(type) {
    const normalized = this.normalizeScalarType(type);
    if (normalized === "const char *") return "\"\"";
  ...
  pushScope() {
    ...
    this.arrayLengthScopes.push(new Map());
  }

  popScope() {
    this.scopes.pop();
    this.arrayLengthScopes.pop();
  }
  ...
  setArrayLength(name, length) {
    if (this.arrayLengthScopes.length === 0) this.pushScope();
    this.arrayLengthScopes[this.arrayLengthScopes.length - 1].set(name, length);
  }
  ...
  lookupArrayLength(name) {
    for (let i = this.arrayLengthScopes.length - 1; i >= 0; i--) {
      if (this.arrayLengthScopes[i].has(name)) return this.arrayLengthScopes[i].get(name);
    }
    return null;
  }
  ...
  emitRecordTypes() {
    ...
    for (const record of this.recordTypes.values()) {
      const fields = record.fields
        ...
        .join("\n");
      definitions.push(`typedef struct ${record.name}\n{\n${fields}\n} ${record.name};`);
    }
