...
class FunctionDecl extends IRNode {
  ...
  static fromJSON(json) {
    return new FunctionDecl(
      ...
      { ...json, async: json.async, generator: json.generator, arrow: json.arrow, expression: json.isExpression || json.expression } // Deserialize async property (support both old and new)
    );
  }
}
...
class Member extends IRNode {
  constructor(object, property, computed = false, options = {}) {
    ...
    // Track whether this is an array access (for bounds checking and 1-based indexing)
    this.isArrayAccess = options.isArrayAccess || false;
    
    // Track if this needs runtime bounds verification
    this.requiresBoundsCheck = options.requiresBoundsCheck || false;
  }
  ...
  toJSON() {
    ...
    if (this.isArrayAccess) json.isArrayAccess = this.isArrayAccess;
    if (this.requiresBoundsCheck) json.requiresBoundsCheck = this.requiresBoundsCheck;
...
class AsyncFunctionDeclaration extends IRNode {
  constructor(id, params, body, options = {}) {
    ...
    // PHASE 4.1: TYPE SYSTEM HARDENING - Validate async function return types
    this._validateAsyncReturnType();
  }

  /**
   * FORENSIC TYPE VALIDATION
   * Ensures async functions have proper return types (Promise/Future, never void)
   * 
   * Rules:
   * - Async functions MUST return Promise<T>, Future<T>, or other awaitable types
   * - Async functions CANNOT return void (invalid in all async-capable languages)
   * - Async functions returning nothing implicitly return Promise<undefined> or Promise<void>
  ...
  _validateAsyncReturnType() {
    ...
    // Rule 3: Validate return type is awaitable
  ...
  /**
   * Check if async function is valid
   */
  isValid() {
