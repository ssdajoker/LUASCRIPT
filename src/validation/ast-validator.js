...
class ASTValidator {
    ...
    visitAsyncFunctionDeclaration(node) {
        if (!node.id || !node.id.name) {
            this.errors.push('AsyncFunctionDeclaration must have named id');
        }
    }
    ...
    visitFunctionExpression(node) {
        if (!node.params || !Array.isArray(node.params)) {
            this.errors.push('FunctionExpression must have params array');
        }
        if (!node.body || node.body.type !== 'BlockStatement') {
            this.errors.push('FunctionExpression body must be BlockStatement');
        }
    }

    visitArrowFunctionExpression(node) {
        if (!node.params || !Array.isArray(node.params)) {
            this.errors.push('ArrowFunctionExpression must have params array');
        }
        // Body can be BlockStatement or Expression
        if (node.expression && typeof node.body !== 'object') {
            this.errors.push('ArrowFunctionExpression expression body must be object');
        }
        if (!node.expression && node.body.type !== 'BlockStatement') {
            this.errors.push('ArrowFunctionExpression block body must be BlockStatement');
        }
    }
    ...
    visitFunctionDeclaration(node) {
        if (!node.id || !node.id.name) {
            this.errors.push('FunctionDeclaration must have named id');
        }
        if (!node.params || !Array.isArray(node.params)) {
