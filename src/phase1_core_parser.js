...
const {
  ...
  ReturnStatementNode, IfStatementNode, WhileStatementNode, DoWhileStatementNode, ForStatementNode, ForOfStatementNode,
  ArrayPatternNode,
  ...
  LogicalExpressionNode, AwaitExpressionNode, YieldExpressionNode, ConditionalExpressionNode, CallExpressionNode,
  ...
  AssignmentPatternNode, RestElementNode, SpreadElementNode,
  ObjectPatternNode,
  ErrorNode
...
class LuaScriptParser {
  ...
  /**
     ...
     * @returns {DoWhileStatementNode} The AST node for the do-while statement.
     * @private
     */
  parseDoWhileStatement() {
    ...
    this.consume("KEYWORD", "Expected 'while' after do-while body");
    if (this.previous().value !== "while") {
      throw new SyntaxError("Expected 'while' after do-while body");
    }

    this.consume("LEFT_PAREN", "Expected '(' after 'while' in do-while");
    const test = this.parseExpression();
    ...
    return new DoWhileStatementNode(body, test, {
      line: token.line,
      column: token.column
    });
  }
  ...
  parseUnaryExpression() {
    // Check for await/yield keywords WITHOUT consuming the token first
    if (this.check("KEYWORD")) {
      const keyword = this.peek().value;
      if (keyword === "await") {
        this.advance(); // Now consume it
        const token = this.previous();
        const argument = this.parseUnaryExpression();
        return new AwaitExpressionNode(argument, { line: token.line, column: token.column });
      }
      if (keyword === "yield") {
        this.advance(); // Now consume it
        const token = this.previous();
        ...
        } else if (!this.check("SEMICOLON") && !this.check("RIGHT_BRACE") && !this.check("EOF")) {
          argument = this.parseUnaryExpression();
        }
        return new YieldExpressionNode(argument, delegate, { line: token.line, column: token.column });
      }
    }
  ...
  parsePrimaryExpression() {
    ...
    if (this.match("LEFT_PAREN")) {
      ...
      // First, try to disambiguate: is this an arrow function or a grouped expression?
      // Arrow functions look like: (params) => body
      // Grouped expressions look like: (expr)
      
      const checkpoint = this.current;
      let couldBeArrow = false;
      
      // Heuristic: look ahead for '=>' to determine if this is likely an arrow function
      let depth = 1;
      let scanPos = this.current;
      while (scanPos < this.tokens.length && depth > 0) {
        const scanToken = this.tokens[scanPos];
        if (scanToken.type === "LEFT_PAREN") depth++;
        if (scanToken.type === "RIGHT_PAREN") depth--;
        if (depth === 0 && scanPos + 1 < this.tokens.length) {
          const nextToken = this.tokens[scanPos + 1];
          if (nextToken.type === "ARROW") {
            couldBeArrow = true;
          }
          break;
        }
      ...
      if (couldBeArrow) {
        try {
          ...
          // If that fails, fall through to grouped expression
  ...
  parseClassDeclaration() {
    ...
    while (!this.check("RIGHT_BRACE") && !this.isAtEnd()) {
      ...
      if (this.check("KEYWORD") && this.peek().value === "async") {
        this.advance();
        isAsync = true;
      }
      ...
      body.push(new (require("./phase1_core_ast").MethodDefinitionNode)(nameId, params, methodBody, { static: isStatic, async: isAsync, kind, line: nameToken.line, column: nameToken.column }));
  ...
  parseParameterList() {
    ...
    if (!this.check("RIGHT_PAREN")) {
      do {
        ...
        params.push(param);
        if (param && param.type === "RestElement" && this.check("COMMA")) {
          throw new SyntaxError(`Rest parameter must be last at line ${this.peek().line}`);
        }
      } while (this.match("COMMA"));
    }
  ...
  /**
     * Parses a single function parameter, including default and rest parameters.
     * @returns {ASTNode} The parsed parameter node.
     * @private
     */
  parseParameter() {
    if (this.match("SPREAD")) {
      const token = this.previous();
      const argument = this.parseBindingIdentifierOrPattern();
      return new RestElementNode(argument, { line: token.line, column: token.column });
    }

    const left = this.parseBindingIdentifierOrPattern();
    if (this.match("ASSIGN")) {
      const right = this.parseAssignmentExpression();
      return new AssignmentPatternNode(left, right, { line: left.line, column: left.column });
    }
  ...
  /**
     * Parses a binding identifier or simple binding pattern.
     * @returns {ASTNode} Identifier or pattern node.
     * @private
     */
  parseBindingIdentifierOrPattern() {
    ...
    if (this.check("LEFT_BRACE") && typeof this.parseObjectPattern === "function") {
    ...
    if (!this.check("RIGHT_PAREN")) {
      do {
        if (this.match("SPREAD")) {
          ...
          args.push(new SpreadElementNode(argument, { line: token.line, column: token.column }));
        } else {
    ...
    if (!this.check("RIGHT_BRACKET")) {
      while (!this.check("RIGHT_BRACKET")) {
        ...
        if (this.match("SPREAD")) {
          const token = this.previous();
          const argument = this.parseBindingIdentifierOrPattern();
          elements.push(new RestElementNode(argument, { line: token.line, column: token.column }));
        } else {
          const element = this.parseBindingIdentifierOrPattern();
          if (this.match("ASSIGN")) {
            const defaultValue = this.parseAssignmentExpression();
            elements.push(new AssignmentPatternNode(element, defaultValue, { line: element.line, column: element.column }));
          } else {
  ...
  /** Parses a simple object pattern for destructuring. */
  parseObjectPattern() {
    const start = this.advance(); // consume '{'
    const properties = [];
    while (!this.check("RIGHT_BRACE") && !this.isAtEnd()) {
      ...
      if (this.match("SPREAD")) {
        const token = this.previous();
        const argument = this.parseBindingIdentifierOrPattern();
        properties.push(new RestElementNode(argument, { line: token.line, column: token.column }));
      } else if (this.match("LEFT_BRACKET")) {
        const key = this.parseExpression();
        this.consume("RIGHT_BRACKET", "Expected ']' after computed property");
        this.consume("COLON", "Expected ':' after computed property key");
        let value = this.parseBindingIdentifierOrPattern();
        if (this.match("ASSIGN")) {
          const defaultValue = this.parseAssignmentExpression();
          value = new AssignmentPatternNode(value, defaultValue, { line: value.line, column: value.column });
        }
        ...
        if (this.match("COLON")) {
          ...
          properties.push(new PropertyNode(key, value, { shorthand: false, line: key.line, column: key.column }));
        } else if (this.match("ASSIGN")) {
          const defaultValue = this.parseAssignmentExpression();
          const value = new AssignmentPatternNode(key, defaultValue, { line: key.line, column: key.column });
          properties.push(new PropertyNode(key, value, { shorthand: true, line: key.line, column: key.column }));
        } else {
          properties.push(new PropertyNode(key, key, { shorthand: true, line: key.line, column: key.column }));
        }
      }
      if (this.match("COMMA")) {
        continue;
    ...
    this.consume("RIGHT_BRACE", "Expected '}' to close object pattern");
    return new ObjectPatternNode(properties, { line: start.line, column: start.column });
  }
