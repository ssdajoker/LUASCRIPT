/**
 * Bash Parser - Script Language
 * Parses bash code to canonical AST for IR pipeline
 */

const { BaseParser } = require("./base_parser");

class BashParser extends BaseParser {
  constructor(source) {
    super(source);
    this.keywords = [
      "function", "if", "then", "else", "elif", "fi", "case", "in", "esac", "for", "while", "do", "done", "return", "exit", "local"
    ];
    this.operators = ["==", "!=", "-eq", "-ne", "-lt", "-le", "-gt", "-ge", "-a", "-o"];
  }
