...
class CoreTranspiler extends EventEmitter {
  ...
  generateLuaFromAST(node) {
    ...
    case "FunctionDeclaration": {
      ...
      if (node.generator) {
        // Generator functions return a generator object with next() method
        const generatorBody = this.indentCode(indentedBody || "", 2);
        functionCode = `local function ${id}(${params})\n` +
                      `  return coroutine.wrap(function()\n${generatorBody}\n  end)\n` +
                      "end";
      } else if (indentedBody) {
        functionCode = `local function ${id}(${params})\n${indentedBody}\nend`;
      } else {
        functionCode = `local function ${id}(${params})\nend`;
      }
      ...
      if (node.async && !node.generator) {
        // Wrap in coroutine.create if it's an async function (but not async generator)
        const coroutineBody = this.indentCode(indentedBody || "", 2);
        functionCode = `local function ${id}(${params})\n` +
    ...
    case "YieldExpression": {
      // Generate coroutine.yield for yield expressions
      if (node.argument) {
        const argument = this.generateLuaFromAST(node.argument);
    ...
    case "AwaitExpression": {
      // Generate await as coroutine.yield for async/await pattern
      const argument = this.generateLuaFromAST(node.argument);
      return `coroutine.yield(${argument})`;
    }
