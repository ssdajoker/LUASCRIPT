 ...
 * Tests: Memory management, parsing correctness, edge cases, consistency
 */
...
function header(title) {
  ...
  console.log(`${COLORS.CYAN}║  ${title.padEnd(74)} ║${COLORS.RESET}`);
...
for (const parserTest of parserTests) {
  ...
  for (const test of parserTest.testCases) {
    try {
      ...
      log(`  ✅ ${test.name.padEnd(20)} | Objects: ${stats.objectCount}`, 'GREEN');
    } catch (error) {
      results[parserTest.name].tests.push({
        name: test.name,
        passed: false,
      ...
      log(`  ❌ ${test.name.padEnd(20)} | ${error.message}`, 'RED');
    }
  }
}
...
for (const parserTest of parserTests) {
  ...
  try {
    ...
    if (tokens.length > 1000) {
      log(`  ⚠️  Token count exceeds 1000 - potential loop risk`, 'YELLOW');
    } else if (duration > 1000) {
      log(`  ⚠️  Tokenization took > 1000ms - potential loop risk`, 'YELLOW');
    } else {
      log(`  ✅ Tokenizer healthy`, 'GREEN');
    }
  } catch (error) {
...
for (const [name, result] of Object.entries(results)) {
  ...
  const status = result.failed === 0 ? '✅ PASS' : '⚠️ PARTIAL';
