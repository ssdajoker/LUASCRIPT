...
**Target**: Complete Phases 1-2 within 2-3 weeks  
**Status**: Ready to execute

---

## 📋 Phase 1: Documentation & Visibility (Week 1)
...
- [x] Update root files to point to `docs/`
  - [x] README.md → link to `docs/INDEX.md` (PENDING)
  - [x] DEVELOPMENT_WORKFLOW.md → link to `docs/development/`
- [x] Commit: "docs: execute wiki consolidation (Codex)"

**Acceptance**: ✅ Wiki structure complete, navigation works, ROOT FILE LINK PENDING (Task 1.2)

**Status**: COMPLETE (link update delegated to Task 1.2)

---

### Task 1.2: Fix README.md Inflation Claims ⏳ IN PROGRESS - PRIORITY NOW
...
- [x] Updated section:
  ...
  ```
- [ ] Verify progress metrics match actual state (90%, 85%, 75%, 60%)
- [ ] Add link to PROJECT_STATUS.md as source of truth
- [ ] Test: `npm run lint` still passes
- [ ] Commit: "docs: remove inflated claims from README, align with PROJECT_STATUS"
...
### Task 1.3: Mark Old Docs as Deprecated ⏳ TODO
- [ ] Identify deprecated docs (20-40 candidates):
  - [ ] Old phase planning docs (PHASE1_COMPLETION_REPORT.md, PHASE_PLAN.md, etc.)
  - [ ] Superseded CI approaches (non-lean CI docs)
  - [ ] Exploration dead-ends (EDGE_CASES.md, etc.)
  - [ ] Outdated status reports (old PROJECT_STATUS versions)
- [ ] For each deprecated doc:
  - [ ] Add frontmatter:
    ```markdown
    # [DEPRECATED - Phase X] Document Title
    
    **Status**: 🔴 DEPRECATED (Superseded 2025-12-20)  
    **See instead**: [Link to current equivalent]  
    **Why it changed**: Brief explanation  
    ```
  - [ ] Add at top of file (before content)
  - [ ] Move to `docs/deprecated/` folder (during wiki setup)
- [ ] Create `docs/deprecated/README.md`:
  - [ ] Explain why these docs exist
  - [ ] How to find current equivalents
  - [ ] Timeline context
- [ ] Create index in `docs/deprecated/INDEX.md`:
  - [ ] List all deprecated docs
  - [ ] Link to current equivalents
  - [ ] Why each was deprecated
- [ ] Test: Links to `docs/deprecated/` work
- [ ] Commit: "docs: mark deprecated docs with [DEPRECATED] prefix and redirect pointers"

**Acceptance**: 20+ docs marked, redirects in place, index complete
...
## 🔧 Phase 2: Lint Burn-down (Weeks 2-3)

### Task 2.1: Apply `_prefix` to Unused Params ⏳ TODO
- [ ] Review current lint output:
  - [ ] Run: `npm run lint 2>&1 | Select-String "Allowed unused args must match"`
  - [ ] Count warnings (should be ~45)
- [ ] Files to update (by priority):
  - [ ] `src/backends/llvm/ir-to-llvm.js` (lines 29, 273)
  - [ ] `src/backends/mlir/dialect.js` (lines 392)
  - [ ] `src/backends/wasm/ir-to-wasm.js` (line 409, 922)
  - [ ] `src/runtime/generator-helpers.js` (line 34)
  - [ ] `src/compilers/ir-to-js.js` (lines 168, 173)
  - [ ] `src/compilers/ir-to-lua.js` (lines 257, 262, 266)
  - [ ] Other files with same pattern (34 more)
- [ ] For each file:
  - [ ] Find function parameter marked as unused
  - [ ] Rename `param` → `_param`
  - [ ] Test: `npm run lint` for that file
- [ ] Run full lint: `npm run lint`
- [ ] Verify: Warnings reduced by ~45 (101 → ~56)
- [ ] Run tests: `npm test` (all pass)
- [ ] Commit: "refactor: prefix unused parameters with _ (ESLint no-unused-vars)"

**Acceptance**: 45 warnings gone, code runs, tests pass, lint shows ~56 warnings

**Est. Time**: 4-5 hours

---

### Task 2.2: Remove Dead Code Assignments ⏳ TODO
- [ ] Identify dead assignments:
  - [ ] `core_transpiler.js:14` - `esprima` assigned but never used
  - [ ] `optimized_transpiler.js` - `fs`, `path` assigned but unused
  - [ ] `enhanced_transpiler.js:16` - `path` assigned but unused
  - [ ] `phase1_core_lexer.js` - `start` assigned but unused (2 places)
  - [ ] `core_transpiler.js` - `idx`, `index` assigned but unused
  - [ ] And ~30 more similar patterns
- [ ] For each:
  - [ ] Decide: Remove, comment, or prefix with `_`?
  - [ ] Apply change
...
### Phase 2 Summary
**Total Time**: 8-12 hours (2 weeks)  
**Result**: Lint warnings reduced 101 → ~16-21 (84% reduction) ✅
