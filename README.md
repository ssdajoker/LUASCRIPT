...
Installed-package consumers should start with `examples/package/transpile-js-to-lua.cjs` or `examples/package/minimal-system.cjs`. Repository developers can also use small JavaScript or `.ls` programs, `tests/actual_programs/fixtures/`, and `examples/supported_math_showcase.ls`. The latter paths are repository-local evidence, not shipped package examples. Avoid treating broad multi-language demos or the full Unicode mathematical DSL as baseline beta proof; those remain experimental unless a current gate names and verifies the slice.

### Known Limitations / Post-Beta Route

Beta v0.1 ships as scoped truth, not as a final summit claim. The former post-beta runtime limitation list has been removed for the current named strict-native slices, and any future setup-blocked lane must stay labeled until its native gate passes. The scoped Denali RC now defines `.ls` identity/profiles, package/runtime expectations, examples, docs, support boundaries, current-host compatibility, and release evidence. Remaining horizons are explicitly authorized release actions, accession-based support expansion, broader semantics/identity proof, and equivalent cross-platform evidence.
...
- `examples/package/transpile-js-to-lua.cjs` and `examples/package/minimal-system.cjs` for the installed package root API.
...
The remaining bullets are repository-local compiler evidence, not package contents. Do not use broad multi-language demos, full Unicode mathematical DSL claims, or unsupported `.ls` profile names as first validation targets. They can become package-facing examples only after a current manifest, runtime gate, docs boundary, package test, and claim check name the slice.
...
- Versioned one-way release-IR compatibility contract, pinned-schema parity, deterministic projections, and malformed-shape negatives: `npm run test:ir-compatibility-bridge`
- Actual-tarball public API/runtime/package contract, clean consumer import/transpile, package hygiene, dependency closure, and Node-floor smoke: `npm run test:package-contract`
...
- Release IR surface contract: [docs/LUASCRIPT_RELEASE_IR_SURFACE_CONTRACT.md](docs/LUASCRIPT_RELEASE_IR_SURFACE_CONTRACT.md)
